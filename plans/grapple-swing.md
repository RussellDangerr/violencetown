...
**Status:** ✅ Shipped 2026-09-10 on `feature/ready-builds` (migrated here from the `plan` branch, where
this was written 2026-07-23 and promoted from `plans/chapter-two-downtown-canyon-and-cohesion.md` §Phase 5).

> **As built — where it differs from the sketch below, and why:**
> - **No `GRAPPLE_ANCHOR` tile.** An anchor is DATA — a map `anchors` list in the transition's own
>   shape — standing on a solid **prop** for its look. That gets a shadow, depth sort and walk-behind
>   for free, lets any future anchor look like anything, and spends no tile id. `GameMap.getAnchor`,
>   one bump case beside the car in `_doMove`, and `_interactGrapple` in `main.js`.
> - **The canyon's anchor is a jutting sandstone spire** (`grappleRock`, roguelikeDungeon (3,0)) — what
>   a hook bites on. Not the sheet's rope-and-hook hoist at (5,9): with a rope already hanging there,
>   you would not need Pike's.
> - **Cross-map anchors keep the exit marker.** The old gated exit was a transition, so it had the gold
>   glow, arrow and hint; `_drawTransitions` now draws them for any anchor with a `toMap` too — without
>   that, the only way out of the canyon would have been unmarked.
> - **The swing:** 2.5× a walk step; the hero rises on an arc peaking at 24px mid-swing while the
>   shadow and depth sort stay on the ground; Reduce Motion flattens it to a slide. A cross-map landing
>   queues the anchor as the pending transition, so pursuit, autosave and `map_entered` all run as for
>   any exit. Same-map anchors (the design's option (a)) are supported and tested; none is placed yet.
> - Verified live: no hook → "Sheer rock…", no move; hook → swing, 24px peak, Downtown (8,10).
>
