...
**Status:** ✅ **DONE / SHIPPED (2026-07-04).** The core (continuous chaining, turn-in-place, `_queuedMoveDir` buffer) shipped earlier. The **held-key resume-fix** (the 2026-07-03 addendum — `_physicalHeld` Set + `_resumeHeldWalk()`, re-armed at every return to live IDLE) **SHIPPED in v0.12.0 (live)** — a held key now keeps walking across a map load / wheel / menu / pause / respawn with no re-press. The **halt-frequency softening for transitions** (removing the `getTransition` line from `_autoRepeatShouldStop`) is **BUILT + on `dev`** (v0.13.0 pending): held-walk now flows smoothly THROUGH a zone exit instead of stopping one tile short and needing a re-tap (Caelan's biggest movement gripe). Walls/hostiles/containers/hazards still halt. `feature/walk-through-transitions` (d5ea92f) was DISCARDED as superseded (its `.then()` re-arm == the more-robust `_resumeHeldWalk`). Remaining truly-optional: softening the *hazard* halt too (not requested).
